$(function () {
    $.get("/householdFurniture/show/getAllFurnitureToZbProList", function (datas) {
        let data = datas.data;
        let code = datas.code;
        if (code == 1000) {
            showFurniture(data);
...
$(".orderBy").click(function () {
    let value = $(this).html();
    $.get("/householdFurniture/show/getAllFurnitureToZbProListOrderBy/" + value, function (datas) {
        let data = datas.data;
        let code = datas.code;
        if (code == 1000) {
            showFurniture(data);
